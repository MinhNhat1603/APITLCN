...
function getProvince(data){
    ...
    for(t; t > -1; t--) { 
        ...
        for( let i=0; i<province.length;i++){
            ...
            for(let j=0; j< name.length;j++){
                if(address[t] == name[j]){
                    codeProvince=district[i].ProvinceID
                    break
                }            
            }
    ...
    console.log(t);
    fetch("https://online-gateway.ghn.vn/shiip/public-api/master-data/district",{method: 'POST', body: JSON.stringify({"province_id": codeDistrict}), 
