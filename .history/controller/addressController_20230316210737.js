const addressController ={
    code: async(req,res)=>{
        try {
            ...
            const Service = await fetch("https://online-gateway.ghn.vn/shiip/public-api/v2/shipping-order/available-services",{method: 'POST', body: JSON.stringify({"shop_id": 3912285, "from_district": 1542 , "to_district": 1542}), headers: {"token": "427bb6dc-bc24-11ed-9dc6-f64f768dbc22"}})
            const dataService = await Service.json();
            var service = dataService.data;
            res.status(200).json(daservice);
        } catch (error) {
