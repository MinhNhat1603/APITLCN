    ...
    addOrder: async(req,res)=>{
        try {
            ...
            for(let i=0; i < cart.length; i++){
                var product = await fetch("http://localhost:3000/product/"+ cart[i].productId, {method: 'GET', headers: {"Content-Type": "application/json"}} )
                var inventorys = parseInt(product.inventory) - parseInt(cart[i].quantity);
                var sold = parseInt(product.sold) + parseInt(cart[i].quantity);
                var results = await fetch("http://localhost:3000/product/"+ cart[i].productId, {method: 'PUT', headers: {"Content-Type": "application/json"},body: JSON.stringify({"inventory": inventorys, "sold": sold})}) 
                if(inventorys!= null){
                    res.status(500).json(parseInt(product.inventory));
                }            
            }
