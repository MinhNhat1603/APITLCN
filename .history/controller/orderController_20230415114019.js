    ...
    addOrder: async(req,res)=>{
        try {
            ...
            for(let i=0; i < cart.length; i++){
                var product = await fetch("http://localhost:5000/product/")
                var results = await fetch("http://localhost:5000/product/"+ cart[0].productId, {method: 'PUT', body: JSON.stringify({"inventory": })})
            }
