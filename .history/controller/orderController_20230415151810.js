...
const orderController = {
    ...
    addOrder: async(req,res)=>{
        try {
            ...
            //for(let i=0; i < cart.length; i++){var product = await fetch("http://localhost:3000/product/"+ cart[0].productId, {method: 'GET'})
                var inventory = parseInt(product.inventory) - parseInt(cart[0].quantity);
                var sold = parseInt(product.sold) + parseInt(cart[i].quantity);
                var results = await fetch("http://localhost:3000/product/"+ cart[0].productId, {method: 'PUT', headers: {"Content-Type": "application/json"},body: JSON.stringify({"inventory": 11, "sold": 22})}) 
                // if(inventory!= null){
                //     res.status(500).json(product.inventory);
                // }
                res.status(500).json(product.inventory);            
            //}
