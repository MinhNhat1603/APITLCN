const addressController ={
    code: async(req,res)=>{
        try {
            ...
            const Service = await fetch("https://online-gateway.ghn.vn/shiip/public-api/v2/shipping-order/available-services",{method: 'POST', headers: {"token": "427bb6dc-bc24-11ed-9dc6-f64f768dbc22"}, body: JSON.stringify({"shop_id": 3912285, "from_district": 1542, "to_district": code[1]})})
            const dataService = await Service.json();
            var service = dataService.data;
            var a =service.data.short_name)
            res.status(200).json(service.data.short_name);
        } catch (error) {
