const addressController ={
    code: async(req,res)=>{
        try {
            ...
            code[1] = getProvince(dataProvince, address,t);
            const District = await fetch("https://online-gateway.ghn.vn/shiip/public-api/master-data/district",{method: 'POST', body: JSON.stringify({"province_id": 202}),headers: {"token": "427bb6dc-bc24-11ed-9dc6-f64f768dbc22", "Content-Type": "application/json"}})
            const dataDistrict = await District.json()
            code[0]=getDistrict(dataDistrict,address,t)
